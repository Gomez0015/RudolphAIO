...
function MEE6Levels(props) {
    ...
    const startFarming = (e) => {
        ...
        if(!e.target) {
          axios.post("https://beta.rudolphaio.com/api/startFarming", {userToken: props.cookies.userToken,token: e.botToken, messageDelay: e.messageDelay, channelId: e.channelId, mintDate: e.mintDate, collectionName: e.collectionName, customPrompt: e.customPrompt, spam: false})
          .then(res => {
    ...
    <div style={{textAlign: 'center'}}>
        ...
        >
          <form action='#' style={{textAlign: 'center'}} autocomplete="off" onSubmit={saveSettings}>
            ...
            <Input autocomplete="off" required type="text" name="channelId" placeholder="Channel Id" defaultValue={botSettings.channelId} onChange={(e) => {botSettings.channelId = e.target.value}}  style={{textAlign: 'center', width: '50%'}}/>
            <br />
            <p style={{marginTop: '30px'}}>Change Message Delay (minutes)</p>
            <Input autocomplete="off" required type="number" name="messageDelay" step="0.1" min='1' placeholder="Message Delay (minutes)" defaultValue={botSettings.messageDelay} onChange={(e) => {botSettings.messageDelay = e.target.value}}  style={{textAlign: 'center', width: '50%'}}/>
            <br />
            <p style={{marginTop: '30px'}}>Change Collection Name</p>
            <Input autocomplete="off" required type="text" name="collectionName" placeholder="Collection Name" defaultValue={botSettings.collectionName} onChange={(e) => {botSettings.collectionName = e.target.value}}  style={{textAlign: 'center', width: '50%'}}/>
            <br />
            <p style={{marginTop: '30px'}}>Change Mint Date (YYYY-MM-DD)</p>
            <Input autocomplete="off" required type="text" name="mintDate" placeholder="Mint Date (YYYY-MM-DD)" defaultValue={botSettings.mintDate} onChange={(e) => {botSettings.mintDate = e.target.value}}  style={{textAlign: 'center', width: '50%'}}/>
            <br />
            <p style={{marginTop: '30px'}}>Custom Prompt</p>
            <TextArea autocomplete="off" type="text" name="customPrompt" defaultValue={botSettings.customPrompt} onChange={(e) => {botSettings.customPrompt = e.target.value}} placeholder="He is chatting in the channel called general. He is from Paris, France and travels very often around the world. He does not like talking about artificial inteligence or robots, he likes talking about fast cars and taking a rocket to the moon or jupiter. His profile picture is his dog, a golden retriever. ect." style={{textAlign: 'center', width: '50%'}}/>
            <br />
