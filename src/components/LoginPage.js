...
function LoginPage(props) {
    ...
    useEffect(() => {
        if(props.cookies.userToken != 'none' && props.cookies.userToken != undefined && props.cookies.userToken != null && newKey == 'false') {
            axios.post('https://beta.rudolphaio.com/api/checkAuthDiscord', {discordId: props.cookies.userToken})
                .then(res => {
    ...
    const Login = async (e) => {
        ...
        axios.post('https://beta.rudolphaio.com/api/checkAuthDiscord', {discordId: discordAuth.data.id, discordLogin: true})
            .then(res => {
    ...
    const Register = async (e) => {
        ...
        axios.post('https://beta.rudolphaio.com/api/linkKeyDiscord', {discordId: discordAuth.data.id, authKey: e.target.authKey.value})
            .then(res => {
                if(res.data.state === 'success') {
                    props.successMessage(res.data.message);
                } else if(res.data.state === 'error'){
    ...
    const CallBack = async (code) => {
        const result = await axios.post('https://beta.rudolphaio.com/api/getDiscordAuthInfo', {code: code});

        return result;
    }

    const DiscordLogin = () => {
        window.open(`https://beta.rudolphaio.com/api/discordLogin`);
    }
