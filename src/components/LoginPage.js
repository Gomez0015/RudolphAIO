...
function LoginPage(props) {
    ...
    useEffect(() => {
        if(props.cookies.userToken != 'none' && props.cookies.userToken != undefined && props.cookies.userToken != null && newKey == 'false') {
            axios.post(process.env.SERVER_URI + '/api/checkAuthDiscord', {discordId: props.cookies.userToken})
                .then(res => {
    ...
    const Login = async (e) => {
        ...
        axios.post(process.env.SERVER_URI + '/api/checkAuthDiscord', {discordId: discordAuth.data.id, discordLogin: true})
            .then(res => {
    ...
    const Register = async (e) => {
        ...
        axios.post(process.env.SERVER_URI + '/api/linkKeyDiscord', {discordId: discordAuth.data.id, authKey: e.target.authKey.value})
            .then(res => {
                if(res.data.state === 'success') {
                    props.successMessage(res.data.message);
                } else if(res.data.state === 'error'){
    ...
    const CallBack = async (code) => {
        const result = await axios.post(process.env.SERVER_URI + '/api/getDiscordAuthInfo', {code: code});

        return result;
    }

    const DiscordLogin = () => {
        window.open(process.env.SERVER_URI + `/api/discordLogin`);
    }
