        ...
        if (data.state == 'error') {
            ...
            await candyMachineScraper.getMetaData(apiKeyId, apiSecretKey, candyId, function(data) {
                ...
                const candyEmbed = new MessageEmbed()
                    ...
                    .addFields({ name: 'Live Date', value: data.data.go_live_date })
                    .addFields({ name: 'Available', value: data.data.items_available })
                    .addFields({ name: 'Minted', value: data.data.items_redeemed })
                    .addFields({ name: 'Price', value: data.data.price })
                    .addFields({ name: 'Symbol', value: data.data.symbol })
                    .setTimestamp()
